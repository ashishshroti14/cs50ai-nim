...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in available_actions:
            if action in [pair[1] for pair in self.q if pair[0] == state]:
                rewards.append(self.q[pair])
        # rewards = [self.q[pair] for pair in self.q if pair[0] == state]

        if len(rewards) == 0 :
            return 0
        # actions = pair[1] for pair in self.q if pair[0] == state
        return max(rewards)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in  available_actions:
            if action in [pair[1] for pair in self.q.items() if pair[0] == state ]:
                if self.q[state, action] == best_future_reward(state):
                    best_action = action
